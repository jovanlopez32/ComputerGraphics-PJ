...
/*  select clearing (background) color       */
    ...
    glClearColor(0.93, 0.96, 0.87, 1.0);
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glFrustum(-1, 1, -1, 1, .95, 10);
...
void display(void)
{
	...
	for(int i = 0; i <= end; i++){ //Object Layer
		...
		for(jt; jt!=array[i].f.end(); jt++){
			...
			glColor3f (jt->I.r, jt->I.g, jt->I.b);
...
void keyboard (unsigned char key, int x, int y)
{
   ...
   switch (key)
   {
      	...
      	case 'w': 
      			//yAxis+=0.1;
      			rotate(array, end, Rf, -0.1, 0, 0);
      			//translate(array, end, 0, 0.1, 0);
        ...
        //Flecha Derecha
      	case 'd': 
      			rotate(array, end, Rf, 0, 0.1, 0);
      			//xAxis+=0.1;
		  		//translate(array, end, 0.1, 0, 0);
   ...
   //gluLookAt(xAxis, yAxis, zAxis, 0, 0, 0, 0, 0, 0);
